...
<!-- 이 부분을 지우고 인증사진을 제출해주세요.-->
![alt text](SQL_adv_week7_3.PNG)

![alt text](SQL_adv_week7_4.PNG)

## 3. GUI 응용 프로그램 

<!-- GUI 응용 프로그램에 관해 배우게 된 점을 적어주세요. -->

`라벨` : 문자를 표현할 수 있는 위젯으로, label 형식을 사용함.    
*pack() 함수 사용해야 화면에 나타남*      
`버튼` : 마우스로 클릭하면 지정한 작업이 실행되도록 사용하는 위젯으로 Button 형식을 사용함.     
*command 옵션으로 사용자가 버튼을 눌렀을 때 지정한 작업을 처리해야한다는 것이 특징임.*     
`위젯의 정렬` : pack() 함수의 옵션 중에서 가로로 정렬하는 방법으로 sdie=LEFT 또는 RIGHT 방식이 있음.      
`위젯 사이에 여백 추가` : pack() 함수의 옵션 중 padx=픽셀값 또는 pady=픽셀값 방식을 사용함.      
`프레임` : 화면 여러 구역으로 나눌 때       
`엔트리` : 입력 상자를 표현함     
`리스트 박스` : 목록을 표현함    

![alt text](SQL_adv_week7_5.PNG)
![alt text](SQL_adv_week7_6.PNG)
---
